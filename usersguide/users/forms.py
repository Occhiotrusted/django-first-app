...
class CustomUserCreationForm(UserCreationForm):
    ...
    first_name = forms.CharField(max_length=50, required=True, widget=forms.EmailInput(
                                 attrs={'class': 'input-register form-control',
                                        'placeholder': 'Youre first name'}))
    
    last_name = forms.CharField(max_length=50, required=True, widget=forms.EmailInput(
                                 attrs={'class': 'input-register form-control',
                                        'placeholder': 'Youre last name'}))
    ...
    marketing_consent1 = forms.BooleanField(required=True,
                                            label='I agree to receive commercial, promotional, and marketing communications.',
                                            widget=forms.CheckboxInput(
                                                attrs={'class': 'checkbox-input-register'}))
    
    marketing_consent2 = forms.BooleanField(required=True,
                                            label='I agree to receive commercial, promotional, and marketing communications.',
                                            widget=forms.CheckboxInput(
                                                attrs={'class': 'checkbox-input-register'}))
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email = email).exists():
            raise forms.ValidationError("This email is already used on our website!")
        return email
...
class CustomUserUpdateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('first_name', 'last_name', 'email', 'address1', 'address2',
                  'city', 'country', 'province', 'zip_code', 'phone')
        widgets = {
            ...
            'zip_code': forms.TextInput(attrs={'class': 'input-register form-control', 
